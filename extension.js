...
function generateCssFile(context) {
	...
	for (let a in byFileType) {
		...
		style += `.tab[title$=".${formatTitle(a)}" i]{background-color:${byFileType[a].backgroundColor} !important; opacity:0.6;}
				.tab[title$="${formatTitle(a)}" i] a,.tab[title$="${formatTitle(a)}" i] .monaco-icon-label:after,.tab[title$="${formatTitle(a)}" i] .monaco-icon-label:before{color:${byFileType[a].fontColor} !important;${fixIconColor(byFileType[a].fontColor)}}`
	...
	for (let a in byDirectory) {
		...
		style += `.tab[title*="${formatTitle(title)}" i]{background-color:${byDirectory[a].backgroundColor} !important; opacity:0.6;}
				.tab[title*="${formatTitle(title)}" i] a,.tab[title*="${formatTitle(title)}" i] .monaco-icon-label:after,.tab[title*="${formatTitle(title)}" i] .monaco-icon-label:before{color:${byDirectory[a].fontColor} !important;${fixIconColor(byDirectory[a].fontColor)}}`
	}
	style += ".tab.active{opacity:1 !important}";
	if (activeTab.backgroundColor != "default") {
		style += `body .tabs-container .tab.active{background-color:${activeTab.backgroundColor} !important; }body .tabs-container .tab.active a,body .tabs-container .tab.active .monaco-icon-label:after,body .tabs-container .tab.active .monaco-icon-label:before{color:${activeTab.fontColor} !important;${fixIconColor(activeTab.fontColor)}}`;
	}
	let activeSelectors = "";
